(function () {
  ...
  function resultsScreen() {
    ...
    const gov = STACKFIT_GOVERNANCE_SUMMARY(state.governance, importantGaps.length === 0);
    ...
    const minFix = blocker ? "Add mandatory human approval before consequential action and obtain a qualified governance review." : importantGaps.length ? importantGaps.map(r => STACKFIT_GAP_CORRECTION(r.cap.id, r.cap.label, r.covering.map(c => c.label), categoryFor(r.cap.id))).join(" ") : unjustified.length ? "Remove or justify overlapping components that do not add resilience, specialisation, validation, or compliance value." : "No material correction required.";
    const overlapHtml = state.overlaps.map(o => `<div class="overlap"><strong>${o.label}: ${o.tools.map(esc).join(" + ")}</strong><p>Reason: ${esc(o.reason === "Other" ? o.other || "Other (not specified)" : o.reason)}</p><p>Estimated impact — Cost: Medium · Latency: Low · Complexity: Medium. ${unjustified.includes(o) ? "Contributes to Overbuilt verdict." : "Does not by itself imply waste."}</p></div>`).join("");
    const overlapDetails = overlapHtml ? `<details><summary>Overlap and redundancy</summary>${overlapHtml}</details>` : "";
